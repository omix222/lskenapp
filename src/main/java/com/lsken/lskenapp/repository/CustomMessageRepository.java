...
public interface CustomMessageRepository extends JpaRepository<CustomMessage, Integer>{
	...
	@Query(value="select * from ( "
			...
			+ "CASE TYPE WHEN 'text' THEN MESSAGE_DETAIL WHEN 'stamp' THEN DATA END AS MESSAGE_DETAIL,"
			+ "TYPE, POST_DATE "
			+ "FROM MESSAGES m "
			+ "LEFT OUTER JOIN STAMPS s ON  (s.FILENAME = m.MESSAGE_DETAIL) , "
			+ "USERS u "
			+ "where m.FROM_USER_ID = u.USER_ID and m.MESSAGE_ID > (?1) "
			+ "ORDER BY POST_DATE ASC "
			+ ") t "
			+ "where rownum <= (?2) "
			+ "order by t.post_Date", 
			nativeQuery=true)
	List<CustomMessage> findLatestLimit(Integer offset, Integer limit);
	
	/**
	 * 指定IDより前のメッセージを取得(指定件数)
	 * @param offset
	 * @param limit
	 * @return
	 */
	@Query(value="select * from ( "
			...
			+ "order by t.post_Date", 
			nativeQuery=true)
	List<CustomMessage> findLatestLimitBefore(Integer offset, Integer limit);
